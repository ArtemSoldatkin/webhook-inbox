...
// RecoverStuckDeliveryAttempts identifies and resets delivery attempts that have been in-flight for too long,
...
// StartRecoveryEngine initializes and starts the delivery recovery engine,
// which periodically checks for stuck delivery attempts and resets them for retrying.
func StartRecoveryEngine(svc *service.Service, interval time.Duration) {
	...
	for {
		...
		if err := recoverStuckDeliveryAttempts(svc); err != nil {
			logrus.
				WithError(err).
				Error("Error recovering stuck delivery attempts")
		}
		<-ticker.C
	}
}
