...
WHERE (
    ...
    AND (
        ...
        OR (
            egress_url ILIKE '%' || @search_query::text || '%'
            OR description ILIKE '%' || @search_query::text || '%'
            OR public_id::text ILIKE '%' || @search_query::text || '%'
        )
    )
    AND (
        @filter_status::text IS NULL
        OR @filter_status::text = ''
        OR @filter_status::text = '*'
...
WHERE (
    ...
    AND (
        ...
        OR @search_query::text = ''
        OR (
            dedup_hash ILIKE '%' || @search_query::text || '%'
            OR method ILIKE '%' || @search_query::text || '%'
            OR ingress_path ILIKE '%' || @search_query::text || '%'
            OR remote_address::text ILIKE '%' || @search_query::text || '%'
        )
    )
ORDER BY CASE
...
WHERE (
    ...
    AND (
        ...
        OR @search_query::text = ''
        OR (
            state ILIKE '%' || @search_query::text || '%'
            OR status_code::text ILIKE '%' || @search_query::text || '%'
            OR error_type ILIKE '%' || @search_query::text || '%'
            OR error_message ILIKE '%' || @search_query::text || '%'
        )
    )
    AND (
        @filter_state::text IS NULL
        OR @filter_state::text = ''
        OR @filter_state::text = '*'
