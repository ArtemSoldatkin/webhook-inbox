...
func listDeliveries(r chi.Router, svc *service.Service) {
	r.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if endpointIDRaw == "" {
			logrus.Error("Missing endpoint_id query parameter")
		...
		if err != nil {
			logrus.WithError(err).Error("Invalid endpoint_id query parameter")
			http.Error(w, "Invalid endpoint_id query parameter", http.StatusBadRequest)
			return
		}
		deliveries, err := svc.ListDeliveries(r.Context(), endpointID)
		if err != nil {
    		logrus.WithError(err).Error("Failed to list deliveries")
			http.Error(w, "Failed to list deliveries", http.StatusInternalServerError)
			return
		}
		response, err := json.Marshal(deliveries)
		if err != nil {
    		logrus.WithError(err).Error("Failed to marshal deliveries")
			http.Error(w, "Failed to list deliveries", http.StatusInternalServerError)
			return
		}
